...
SELECT TRIM(STR(Id)) AS Id, CAST(ActivityDate AS date) AS ActivityDate, DATENAME(WEEKDAY,
	ActivityDate) AS day_of_week, TotalSteps, VeryActiveMinutes, FairlyActiveMinutes, 
...
SELECT TRIM(STR(fun.Id)) AS Id, CAST(fun.activitydate AS date) AS ActivityDate, fun.day_of_week, 
	fun.VeryActiveMinutes, fun.FairlyActiveMinutes, fun.LightlyActiveMinutes, fun.SedentaryMinutes,
...
SELECT *
FROM Bellabeat..baseSleepDay
WHERE  ID IS NULL OR Sleepday IS NULL OR TotalSleepRecords IS NULL 
	   OR TotalMinutesAsleep IS NULL OR TotalTimeInBed IS NULL;
-- No NULL values found
...
SELECT day_of_week, CAST(AVG(VeryActiveMinutes) AS int) AS AvgActiveTime, 
	CAST(AVG(SedentaryMinutes) AS int) AS SedentaryTime
FROM Bellabeat..DailyActivity
GROUP BY day_of_week;


--Calculate time device is worn each day
SELECT day_of_week, CAST(AVG(TotalTime) AS INT) AS TotalTime, 
...
WITH DIF_CTE AS (
	SELECT CAST(AVG(VeryActiveMinutes) AS int) AS AvgVAct, CAST(AVG(FairlyActiveMinutes) AS int) AS AvgFAct, 
	CAST(AVG(LightlyActiveMinutes) AS int) AS AvgLAct, CAST(AVG(SedentaryMinutes) AS int) AS AvgSed, 
	CAST(AVG(TotalTimeInBed) AS int) AvgBed
	FROM Bellabeat..SleepDay
	)
...
FROM (
	SELECT day_of_week, CAST(AVG(TotalTimeInBed) AS int) AS TotalTimeInBed, 
		CAST(AVG(TotalMinutesAsleep) AS int) AS TotalMinutesAsleep
	FROM Bellabeat..SleepDay
	GROUP BY day_of_week
	) sub
GROUP BY day_of_week
